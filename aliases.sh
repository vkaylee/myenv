...
if [ "$(command -v docker)" ]; then
    ...
    set_command_alias 'gcloud' "docker run --rm -ti --name gcloud -e CLOUDSDK_CONFIG=/config/mygcloud -v ${work_dir}/appconfig/google_cloud/mygcloud:/config/mygcloud -v ${work_dir}/appconfig/google_cloud:/certs gcr.io/google.com/cloudsdktool/google-cloud-cli gcloud"
    set_command_alias 'gcli' 'gcloud'
fi
