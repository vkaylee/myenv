...
if [ "$(command -v docker)" ]; then
    ...
    if [[ -z "${AWS_ACCESS_KEY_ID}" || -z "${AWS_SECRET_ACCESS_KEY}" || -z "${AWS_DEFAULT_REGION}" ]]; then
        ...
        myenv_lib_983459816_set_command_aliases 'awscli,aws' 'myenv_lib_983459816-docker_exec run --rm -it -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e AWS_DEFAULT_REGION public.ecr.aws/aws-cli/aws-cli:latest' 'Amazon Web Service'
    fi

    # Add terraform
    myenv_lib_983459816_set_command_aliases 'terraform,tf' "myenv_lib_983459816-docker_exec run --rm -ti --name terraform -w /workspace -v \$(pwd):/workspace -v ${MYENV_APPCONFIG_DIR}/terraform:/root/.terraform.d hashicorp/terraform:latest" 'Infrastructure as code (IAC) tool'
    # Add google cloud cli
    myenv_lib_983459816_set_command_aliases 'gcloud,gcli' "myenv_lib_983459816-docker_exec run --rm -ti --name gcloud -e CLOUDSDK_CONFIG=/config/mygcloud -v ${MYENV_APPCONFIG_DIR}/google_cloud/mygcloud:/config/mygcloud -v ${MYENV_APPCONFIG_DIR}/google_cloud:/certs gcr.io/google.com/cloudsdktool/google-cloud-cli gcloud" 'Google cloud command line tool'
    myenv_lib_983459816_set_command_aliases 'gshell' 'gcloud cloud-shell ssh --authorize-session' 'Google cloud shell'
    myenv_lib_983459816_set_command_aliases 'flyctl,flyio,fly' "myenv_lib_983459816-docker_exec run --rm -ti --name flyio -v ${MYENV_APPCONFIG_DIR}/flyio:/.fly flyio/flyctl" 'Fly is a platform for running full stack apps and databases close to your users'
    # Add mitmproxy
